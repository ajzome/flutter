...
Future<(Uri? nativeAssetsYaml, List<Uri> dependencies)> buildNativeAssetsLinux({
  ...
  final Uri? absolutePath = flutterTester ? buildUri_ : null;
  final Map<Asset, Asset> assetTargetLocations = _assetTargetLocations(nativeAssets, absolutePath);
  final Map<AssetPath, List<Asset>> fatAssetTargetLocations = _fatAssetTargetLocations(
    nativeAssets,
    absolutePath,
  );
  await copyNativeAssetsLinuxHost(
    buildUri_,
...
Map<AssetPath, List<Asset>> _fatAssetTargetLocations(
  ...
  for (final Asset asset in nativeAssets) {
    final AssetPath path = _targetLocationLinux(asset, absolutePath).path;
    result[path] ??= <Asset>[];
...
Future<void> copyNativeAssetsLinuxHost(
  Uri buildUri,
  ...
  if (assetTargetLocations.isNotEmpty) {
    ...
    for (final MapEntry<AssetPath, List<Asset>> assetMapping in assetTargetLocations.entries) {
      ...
      final List<Uri> sources = <Uri>[
        for (final Asset source in assetMapping.value) (source.path as AssetAbsolutePath).uri,
      ];
      final Uri targetUri = buildUri.resolveUri(target);
      final String targetFullPath = targetUri.toFilePath();
      await fileSystem.file(sources.single).copy(targetFullPath);
    }
