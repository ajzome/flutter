...
void _updateGeneratedXcodePropertiesFile({
  ...
  final File generatedXcodePropertiesFile = useMacOSConfig
      ...
      : project.xros.generatedXcodePropertiesFile;
...
void _updateGeneratedEnvironmentVariablesScript({
  ...
  final File generatedModuleBuildPhaseScript = useMacOSConfig
      ...
      : project.xros.generatedEnvironmentVariableExportScript;
...
String? parsedBuildName({
  ...
  return validatedBuildNameForPlatform(TargetPlatform.xros, buildNameToParse, globals.logger);
}
...
Future<List<String>> _xcodeBuildSettingsLines({
  ...
  if (usingCoreDevice && !buildInfo.isRelease) {
    xcodeBuildSettings.add('BUILD_DIR=${globals.fs.path.absolute(getXrosBuildDirectory())}');
  }
  ...
  if (localEngineInfo != null) {
    ...
    // the conventions uses in the engine: 32-bit visionOS engines are built to
    ...
    if (useMacOSConfig) {
      ...
      if (localEngineName.endsWith('_arm')) {
        throwToolExit('32-bit visionOS local engine binaries are not supported.');
  ...
  if (!useMacOSConfig) {
    ...
    // VisionOS has never been 32bit; left as reference
    // String excludedSimulatorArchs = 'i386';
    // if (!(await project.xros.pluginsSupportArmSimulator())) {
    //   excludedSimulatorArchs += ' arm64';
    // }
    // xcodeBuildSettings.add('EXCLUDED_ARCHS[sdk=xrsimulator*]=$excludedSimulatorArchs');
    // xcodeBuildSettings.add('EXCLUDED_ARCHS[sdk=xros*]=armv7');
  }
