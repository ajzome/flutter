...
/// Tests whether a [Directory] is an visionOS bundle directory.
bool _isBundleDirectory(Directory dir) => dir.path.endsWith('.app');

abstract class XROSApp extends ApplicationPackage {
  XROSApp({required String projectBundleId}) : super(id: projectBundleId);

  /// Creates a new XROSApp from an existing app bundle or IPA.
  static XROSApp? fromPrebuiltApp(FileSystemEntity applicationBinary) {
    ...
    if (entityType == FileSystemEntityType.directory) {
      ...
      if (!payloadDir.existsSync()) {
        globals.printError(
            'Invalid prebuilt visionOS ipa. Does not contain a "Payload" directory.');
        return null;
      }
      try {
        ...
        globals.printError(
            'Invalid prebuilt visionOS ipa. Does not contain a single app bundle.');
        return null;
      }
    }
    final String plistPath = globals.fs.path.join(uncompressedBundle.path, 'Info.plist');
    if (!globals.fs.file(plistPath).existsSync()) {
      globals.printError('Invalid prebuilt visionOS app. Does not contain Info.plist.');
      return null;
    }
    ...
    if (id == null) {
      globals.printError('Invalid prebuilt visionOS app. Info.plist does not contain bundle identifier');
      return null;
    }

    return PrebuiltXROSApp(
  ...
  static Future<XROSApp?> fromXrosProject(XrosProject project, BuildInfo? buildInfo) async {
    ...
    if (!project.xcodeProject.existsSync()) {
      globals.printError('Expected xros/Runner.xcodeproj but this file is missing.');
      return null;
    }
    if (!project.xcodeProjectInfoFile.existsSync()) {
      globals.printError('Expected xros/Runner.xcodeproj/project.pbxproj but this file is missing.');
      return null;
    }
    return BuildableXROSApp.fromProject(project, buildInfo);
  }
  ...
  /// Directory used by xros-deploy to store incremental installation metadata for
  /// faster second installs.
  Directory? get appDeltaDirectory;
}

class BuildableXROSApp extends XROSApp {
  BuildableXROSApp(this.project, String projectBundleId, String? hostAppBundleName)
      : _hostAppBundleName = hostAppBundleName,
        super(projectBundleId: projectBundleId);

  static Future<BuildableXROSApp?> fromProject(XrosProject project, BuildInfo? buildInfo) async {
    ...
    if (projectBundleId != null) {
      return BuildableXROSApp(project, projectBundleId, hostAppBundleName);
    }
    return null;
  }

  final XrosProject project;
  ...
  @override
  String get simulatorBundlePath => _buildAppPath('visionsimulator');

  @override
  String get deviceBundlePath => _buildAppPath('visionos');

  @override
  Directory get appDeltaDirectory => globals.fs.directory(globals.fs.path.join(getXrosBuildDirectory(), 'app-delta'));

  // Xcode uses this path for the final archive bundle location,
  // not a top-level output directory.
  // Specifying `build/xros/archive/Runner` will result in `build/xros/archive/Runner.xcarchive`.
  String get archiveBundlePath => globals.fs.path.join(getXrosBuildDirectory(), 'archive',
      _hostAppBundleName == null ? 'Runner' : globals.fs.path.withoutExtension(_hostAppBundleName!));

  // The output xcarchive bundle path `build/xros/archive/Runner.xcarchive`.
  ...
  String get ipaOutputPath =>
      globals.fs.path.join(getXrosBuildDirectory(), 'ipa');

  String _buildAppPath(String type) {
    return globals.fs.path.join(getXrosBuildDirectory(), type, _hostAppBundleName);
  }

  String _projectImageAssetDirName(String asset)
  => globals.fs.path.join('xros', 'Runner', 'Assets.xcassets', asset);
...
class PrebuiltXROSApp extends XROSApp implements PrebuiltApplicationPackage {
  PrebuiltXROSApp({
  ...
  ///
  /// [XROSApp.fromPrebuiltApp] will uncompress the application into a temporary
  /// directory even when an `.ipa` file was used to create the [XROSApp] instance.
