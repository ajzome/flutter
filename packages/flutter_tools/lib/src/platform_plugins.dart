...
/// Contains the parameters to template an visionOS (xros) plugin.
///
/// The [name] of the plugin is required. Additionally, either:
/// - [defaultPackage], or
/// - an implementation consisting of:
...
///     Dart code
/// is required.
class XROSPlugin extends PluginPlatform implements NativeOrDartPlugin, DarwinPlugin {
  const XROSPlugin({
    required this.name,
    required this.classPrefix,
    this.pluginClass,
  ...
  factory XROSPlugin.fromYaml(String name, YamlMap yaml) {
    assert(validate(yaml)); // TODO(zanderso): https://github.com/flutter/flutter/issues/67241
    return XROSPlugin(
      name: name,
      classPrefix: '',
  ...
  static const String kConfigKey = 'xros';
  ...
  /// Indicates the visionOS native code is shareable with macOS in
  /// the subdirectory "darwin", otherwise in the subdirectory "xros".
  @override
  final bool sharedDarwinSource;
